...
export class DictionaryLegacy<K extends string | number | object, V> extends ObjectBase implements IDictionary<K, V> {
    ...
    private checkKeyAndPerformAction(action: { (key: K, value?: V): void | V | boolean }, key: K, value?: V): void | V | boolean {
        if (DictionaryLegacy.isEitherUndefinedNullOrStringEmpty(key)) {
            throw new Error(DictionaryLegacy._undefinedKeyErrorMessage);
        }
        return action(key, value);
    }

    static Cast<K extends string | number | object, V>(obj: any) { return obj as DictionaryLegacy<K, V> };
    static CastArray<K extends string | number | object, V>(obj: any) { return obj as Array<DictionaryLegacy<K, V>> };
